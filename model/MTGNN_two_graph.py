...
class gtnet(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=10, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True, is_classifier=False, cell_name='LSTM'):
        ...
        self.gc_batch = graph_constructor_lstm(num_nodes, subgraph_size, node_dim, device, alpha=tanhalpha, static_feat=static_feat, cell_name=cell_name)
        self.cls_emb = None
        ...
        for i in range(1):
            ...
            for j in range(1,layers+1):
                ...
                if self.gcn_true:
                    self.gconv1.append(mixprop_two_graph(conv_channels, residual_channels, gcn_depth, dropout, propalpha))
                    self.gconv2.append(mixprop_two_graph(conv_channels, residual_channels, gcn_depth, dropout, propalpha))
    ...
    def forward(self, input, idx=None, CLS=False):
        ...
        if self.gcn_true:
            if self.buildA_true:
                if idx is None:
                    adp = self.gc(None, self.idx)
                    adp_batch = self.gc_batch(input.squeeze().permute(0,2,1), self.idx)
                else:
                    adp = self.gc(idx)
            else:
        ...
        for i in range(self.layers):
            ...
            skip = s + skip
            adp_batch_transpose = adp_batch.permute(0, 2, 1)
            adp_transpose = adp.permute(1, 0)
            if self.gcn_true:
                x = self.gconv1[i](x, adp, adp_batch)+self.gconv2[i](x, adp_transpose, adp_batch_transpose)
            else:
