...
class gtnet(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=10, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True, is_classifier=False):
        ...
        if self.is_classifier:
            self.linear = nn.Linear(in_features=end_channels*num_nodes, out_features=2*out_dim)
        self.gconv1 = prop(seq_length, conv_channels, 1, dropout, propalpha)
        self.gconv2 = prop(conv_channels, end_channels, 1, dropout, propalpha)
    ...
    def forward(self, input, idx=None, CLS=False):
        ...
        output_dict['cls_emb'] = None
        if self.is_classifier:
            print('x.shape', x.shape)
            x = x.reshape(x.shape[0], -1)
            x = self.linear(x).reshape(x.shape[0], self.out_dim, 2)
            
            output_dict['classification_output'] = x
