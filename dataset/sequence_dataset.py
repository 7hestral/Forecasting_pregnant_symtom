...
class MultiStepDataset(Dataset):
    def __init__(self, data, horizon, window, device, user_id, sym_idx=-1):
    ...
    def _get_change_mask(self, sym_labels_x, sym_labels_y):
        if self.horizon == 2:
            weight = torch.zeros((2))
            
            if sym_labels_x[-1] != sym_labels_y[0]:
                weight[0] = 1
            if sym_labels_y[0] != sym_labels_y[1]:
                weight[1] = 1
            return weight
        elif self.horizon == 1:
    ...
    def _batchify(self, idx_set):
        ...
        Y = torch.zeros((n, self.horizon, self.feature_size))
        for i in range(n):
            end = idx_set[i] - self.horizon + 1
            start = end - self.window
            X[i, :, :] = torch.from_numpy(self.data[start:end, :])
            Y[i, :, :] = torch.from_numpy(self.data[end:idx_set[i]+1, :])
        return [X, Y]
    ...
    def __getitem__(self, idx):
        output_dict = {"X": self.data_X[idx, :, :], 
                       
                       "Y": self.data_Y[idx, :, :], 
                       'change_mask': self._get_change_mask(self.data_X[idx, :, -1], self.data_Y[idx, :, -1]),
