...
import pandas as pd
import numpy as np
# from . import s3_utils.loading_method
...
def data_load(data_keys={'oura_sleep', 'birth'}, wave=4):
    ...
    key = f'bump/redcap/study_ids.csv.gz'
    loading_method = pandas_from_csv_vector
    df_studyID = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'birth' in data_keys:
        key = f'bump/redcap/birthing_data.csv.gz'
        df_birth = loading_method(bucket, key=key, compression='gzip')
        ...
        print(df_birth.head())
        df_birth['user_id'] = df_birth.user_id_x
    ...
    if 'bodyport' in data_keys:
        key = f'bump/bodyport/bodyport.csv.gz'
        df_bodyport = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'oura_sleep' in data_keys:
        key = f'bump/oura/oura_sleep.csv.gz'
        df_sleep = loading_method(bucket, key=key, compression='gzip')
        df_sleep['date'] = pd.to_datetime(df_sleep.event_date).dt.date
        dfs['oura_sleep'] = df_sleep
    
    if 'oura_activity' in data_keys:
        key = f'bump/oura/oura_activity.csv.gz'
        df_activity = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'oura_readiness' in data_keys:
        key = f'bump/oura/oura_readiness.csv.gz'
        df_readiness = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'camcog_EBT' in data_keys:
        key = f'bump/camcog/EBT.csv.gz'
        df_EBT = loading_method(bucket, key=key, compression='gzip')
        df_EBT['date'] = pd.to_datetime(df_EBT.event_date).dt.date
        dfs['camcog_EBT'] = df_EBT

    if 'camcog_NBX' in data_keys:
        key = f'bump/camcog/NBX.csv.gz'
        df_NBX = loading_method(bucket, key=key, compression='gzip')
        # df_NBX['date'] = pd.to_datetime(df_NBX.event_date).dt.date
        dfs['camcog_NBX'] = df_NBX

    if 'camcog_PVT' in data_keys:
        key = f'bump/camcog/PVT.csv.gz'
        df_PVT = loading_method(bucket, key=key, compression='gzip')
        # df_PVT['date'] = pd.to_datetime(df_PVT.event_date).dt.date
        dfs['camcog_PVT'] = df_PVT
    
    # Garmin Data
    if 'garmin_activities' in data_keys:
        key = f'bump/garmin/garmin_activities.csv.gz'
        df_garmin_activities = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_activity_details' in data_keys:
        key = f'bump/garmin/garmin_activity_details.csv.gz'
        df_garmin_activity_details = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_dailies' in data_keys:
        key = f'bump/garmin/garmin_dailies.csv.gz'
        df_garmin_dailies = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_epochs' in data_keys:
        key = f'bump/garmin/garmin_epochs.csv.gz'
        df_garmin_epochs = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_moveiq' in data_keys:
        key = f'bump/garmin/garmin_moveiq.csv.gz'
        df_garmin_moveiq = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_pulse_ox' in data_keys:
        key = f'bump/garmin/garmin_pulse_ox.csv.gz'
        df_garmin_pulse_ox = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_respiration' in data_keys:
        key = f'bump/garmin/garmin_respiration.csv.gz'
        df_garmin_respiration = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_sleep' in data_keys:
        key = f'bump/garmin/garmin_sleep.csv.gz'
        df_garmin_sleep = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_stress_details' in data_keys:
        key = f'bump/garmin/garmin_stress_details.csv.gz'
        df_garmin_stress_details = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'garmin_user_metrics' in data_keys:
        key = f'bump/garmin/garmin_user_metrics.csv.gz'
        df_garmin_user_metrics = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'surveys' in data_keys:
        key = f'bump/app_activities/surveys.csv.gz'
        df_survey = loading_method(bucket, key=key, compression='gzip')
        df_survey['date'] = pd.to_datetime(df_survey.updated_at).dt.date
        dfs['surveys'] = df_survey

    # SAM survey
    if 'quick_activities' in data_keys:
        key = f'bump/app_activities/quick_activities.csv.gz'
        df_sam = loading_method(bucket, key=key, compression='gzip')
        df_sam['date'] = pd.to_datetime(df_sam.event_date).dt.date
        dfs['quick_activities'] = df_sam

    # Others
    if 'ace' in data_keys:
        key = f'bump/redcap/ace.csv.gz'
        df_ace = loading_method(bucket, key=key, compression='gzip')
        dfs['ace'] = df_ace
    if 'adverse_event_report' in data_keys:
        key = f'bump/redcap/adverse_event_report.csv.gz'
        df_adverse_event_report = loading_method(bucket, key=key, compression='gzip')
        dfs['adverse_event_report'] = df_adverse_event_report
    if 'birthing_data_cohort_2_only' in data_keys:
        key = f'bump/redcap/birthing_data_cohort_2_only.csv.gz'
        df_birthing_data_cohort_2_only = loading_method(bucket, key=key, compression='gzip')
        dfs['birthing_data_cohort_2_only'] = df_birthing_data_cohort_2_only
    if 'check_in_2_addendum' in data_keys:
        key = f'bump/redcap/check_in_2_addendum.csv.gz'
        df_check_in_2_addendum = loading_method(bucket, key=key, compression='gzip')
        dfs['check_in_2_addendum'] = df_check_in_2_addendum
    if 'check_in_adherence_log' in data_keys:
        key = f'bump/redcap/check_in_adherence_log.csv.gz'
        df_check_in_adherence_log = loading_method(bucket, key=key, compression='gzip')
        dfs['check_in_adherence_log'] = df_check_in_adherence_log
    if 'cssrs' in data_keys:
        key = f'bump/redcap/cssrs.csv.gz'
        df_cssrs = loading_method(bucket, key=key, compression='gzip')
        dfs['cssrs'] = df_cssrs
    if 'enrollment_status' in data_keys:
        key = f'bump/redcap/enrollment_status.csv.gz'
        df_enrollment_status = loading_method(bucket, key=key, compression='gzip')
        dfs['enrollment_status'] = df_enrollment_status
    if 'generalized_anxiety_disorder_scale_gad7' in data_keys:
        key = f'bump/redcap/generalized_anxiety_disorder_scale_gad7.csv.gz'
        df_generalized_anxiety_disorder_scale_gad7 = loading_method(bucket, key=key, compression='gzip')
        dfs['generalized_anxiety_disorder_scale_gad7'] = df_generalized_anxiety_disorder_scale_gad7
    if 'medications' in data_keys:
        key = f'bump/redcap/medications.csv.gz'
        df_medications = loading_method(bucket, key=key, compression='gzip')
        dfs['medications'] = df_medications
    if 'metadata' in data_keys:
        key = f'bump/redcap/metadata.csv.gz'
        df_metadata = loading_method(bucket, key=key, compression='gzip')
        dfs['metadata'] = df_metadata
    if 'monthly_points_log' in data_keys:
        key = f'bump/redcap/monthly_points_log.csv.gz'
        df_monthly_points_log = loading_method(bucket, key=key, compression='gzip')
        dfs['monthly_points_log'] = df_monthly_points_log
    if 'personal_characteristics' in data_keys:
        key = f'bump/redcap/personal_characteristics.csv.gz'
        df_personal_characteristics = loading_method(bucket, key=key, compression='gzip')
        dfs['personal_characteristics'] = df_personal_characteristics
    if 'phq9' in data_keys:
        key = f'bump/redcap/phq9.csv.gz'
        df_phq9 = loading_method(bucket, key=key, compression='gzip')
        dfs['phq9'] = df_phq9
    if 'pitting_edema' in data_keys:
        key = f'bump/redcap/pitting_edema.csv.gz'
        df_pitting_edema_cohort_2_only = loading_method(bucket, key=key, compression='gzip')
        dfs['pitting_edema'] = df_pitting_edema_cohort_2_only

    # if 'pitting_edema_cohort_2_only' in data_keys:
    #     key = f'bump/redcap/pitting_edema_cohort_2_only.csv.gz'
    #     df_pitting_edema_cohort_2_only = loading_method(bucket, key=key, compression='gzip')
    #     dfs['pitting_edema_cohort_2_only'] = df_pitting_edema_cohort_2_only
    if 'ehr_data_extraction' in data_keys:
        key = f'bump/redcap/ehr_data_extraction.csv.gz'
        df_ehr = loading_method(bucket, key=key, compression='gzip')
        dfs['ehr_data_extraction'] = df_ehr
    if 'risk_alert_trigger' in data_keys:
        key = f'bump/redcap/risk_alert_trigger.csv.gz'
        df_risk_alert_trigger = loading_method(bucket, key=key, compression='gzip')
        dfs['risk_alert_trigger'] = df_risk_alert_trigger
    if 'study_completion_form' in data_keys:
        key = f'bump/redcap/study_completion_form.csv.gz'
        df_study_completion_form = loading_method(bucket, key=key, compression='gzip')
        dfs['study_completion_form'] = df_study_completion_form
    if 'study_dates_cohort_info' in data_keys:
        key = f'bump/redcap/study_dates_cohort_info.csv.gz'
        df_study_dates_cohort_info = loading_method(bucket, key=key, compression='gzip')
        dfs['study_dates_cohort_info'] = df_study_dates_cohort_info
    if 'study_ids' in data_keys:
        key = f'bump/redcap/study_ids.csv.gz'
        df_study_ids = loading_method(bucket, key=key, compression='gzip')
        dfs['study_ids'] = df_study_ids
    if 'suicide_risk_response_completed_by_study_doctor' in data_keys:
        key = f'bump/redcap/suicide_risk_response_completed_by_study_doctor.csv.gz'
        df_suicide_risk_response_completed_by_study_doctor = loading_method(bucket, key=key, compression='gzip')
    ...
    if 'symptom_tracking_permission' in data_keys:
        key = f'bump/redcap/symptom_tracking_permission.csv.gz'
        df_symptom_tracking_permission = loading_method(bucket, key=key, compression='gzip')
